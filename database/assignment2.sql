-- 1. Insert Tony, Stark, tony@starkent.com, Iam1ronM@n into the account table
...
-- 5. Use an inner join to select the make and model fields from the 
        -- inventory table and the classification name field from the 
        -- classification table for inventory items that belong to the 
        -- "Sport" category.
    SELECT inv_make AS make, inv_model AS model, classification_name AS class
    FROM inventory inv
